//
//  ShippingOrderTableViewController.swift
//  SHTransactionApp
//
...
class ShippingOrderTableViewController: UITableViewController {
    var orders:[Order] = []
    ...
    func loadShippingOrders(){
        db.collection(T.FStore.Collection.orderCollection).whereField(T.FStore.Order.seller, isEqualTo: Auth.auth().currentUser?.email).addSnapshotListener { (snapshot, error) in
            self.orders = []
            if let e = error {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case T.FStore.Order.statusPending:
            performSegue(withIdentifier: T.Identifier.orderDetailSegue, sender: self)
        case T.FStore.Order.statusShipping:
            performSegue(withIdentifier: T.Identifier.shippedTrackSegue, sender: self)
